...
namespace ALISS.HISUpload.Library
{
    public interface IHISFileUploadService
    {
        ...
        List<STGLabFileDataDetailDTO> GetLabDataWithHISDetail(LabDataWithHISSearchDTO searchModel);
        List<HISDetailDTO> GetSTGHISUploadDetail(LabDataWithHISSearchDTO searchModel);
        byte[] ExportLabDataWithHIS(LabDataWithHISSearchDTO model);
        byte[] ExportHIS_WithStore(TRSPWithHISSearchDTO model);
    }
}
