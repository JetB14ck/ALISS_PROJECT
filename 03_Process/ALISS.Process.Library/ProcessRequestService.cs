...
//using ALISS.TR.NoticeMessage.DataService;
using AutoMapper;
...
namespace ALISS.Process.Library
{
    public class ProcessRequestService : IProcessRequestService
    {
        ...
        //private readonly NoticeMessageDataService _noti;
        ...
        public ProcessRequestService(ProcessContext db, IMapper mapper)
        {
            _db = db;
            _mapper = mapper;
            //_noti = new NoticeMessageDataService(mapper);
        }
        ...
        public List<ProcessRequestDTO> GetListWithModel(ProcessRequestDTO searchModel)
        {
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    objList = _db.ProcessRequestDTOs.FromSqlRaw<ProcessRequestDTO>("sp_GET_TRProcessRequest_DTO {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}", searchModel.pcr_code, searchModel.pcr_arh_code, searchModel.pcr_prv_code, searchModel.pcr_hos_code, searchModel.pcr_month_start, searchModel.pcr_month_end, searchModel.pcr_year, searchModel.pcr_type).ToList();
        ...
        public List<ProcessRequestCheckDetailDTO> GetCheckDetailListWithParam(ProcessRequestDTO searchModel)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    var objReturnList = _db.ProcessRequestDetailDTOs.FromSqlRaw<ProcessRequestDetailDTO>("sp_GET_CheckTRProcessRequestDetail_DTO {0}, {1}, {2}, {3}, {4}, {5}, {6}", "C", null, searchModel.pcr_arh_code, searchModel.pcr_prv_code, searchModel.pcr_hos_code, searchModel.pcr_type, searchModel.pcr_year).ToList();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (objModel != null)
                    {
                        ...
                        if (objModel.pcr_type == "01")
                        {
                            mnu = "MNU_0704";
                            msg = "NARST : " + (objModel.pcr_month_start + "/" + objModel.pcr_year + " ") + " - " + (objModel.pcr_month_end + "/" + objModel.pcr_year + " ") + "set public";
                        }
                        else if (objModel.pcr_type == "02")
                        {
                            mnu = "MNU_0708";
                            msg = "GLASS : " + (objModel.pcr_month_start + "/" + objModel.pcr_year + " ") + " - " + (objModel.pcr_month_end + "/" + objModel.pcr_year + " ") + "set public";
                        }

                        //_noti.CreateNoticeMessageAll(mnu, msg);
