...
namespace ALISS.Master.Library
{
    public class TRNoticeMessageService : ITRNoticeMessageService
    {
        ...
        public List<TRNoticeMessageDTO> GetList(string username)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    objList = _db.TRNoticeMessageDTOs.FromSqlRaw<TRNoticeMessageDTO>("sp_GET_TRNoticeMessage {0}", username).ToList();
        ...
        public List<TRNoticeMessageDTO> GetListWithModelByRole(TRNoticeMessageDTO searchModel)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    objList = _db.TRNoticeMessageDTOs.FromSqlRaw<TRNoticeMessageDTO>("sp_GET_TRNoticeMessageByRole {0},{1},{2}"
                                                                                                        , searchModel.noti_username
                                                                                                        , searchModel.noti_rol_code
