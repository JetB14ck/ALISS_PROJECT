...
namespace ALISS.Data.D6_Report.Antibiogram
{
    public class AntibiogramTemplateService
    {
        ...
        public async Task<List<AntibiogramProvinceTemplateDTO>> GetAntibiogramProvinceTemplateListModelAsync(AntiHospitalSearchDTO searchData)
        {
            List<AntibiogramProvinceTemplateDTO> objList = new List<AntibiogramProvinceTemplateDTO>();

            objList = await _apiHelper.GetDataListByModelAsync<AntibiogramProvinceTemplateDTO, AntiHospitalSearchDTO>("antitemplate_api/GetAntiProvinceTemplateModel", searchData);
        ...
        {
            var filename = selectedobj.file_name;
            ...
            if (objParamList.FirstOrDefault(x => x.prm_code_minor == "PATH") != null)
            {
                ...
                statuscode = await _apiHelper.ConvertResultToPdfDataAsync("exportpdf_api/PdfGenerator", lstfullname);
        ...
        public async Task<string> DownloadPDFFileAsync(AntibiogramProvinceTemplateDTO selectedobj)
        {
            //var filename = selectedobj.file_path.Remove(0, 1) + "/" + selectedobj.file_name;
            var filename = selectedobj.file_name;
            var extension = Path.GetExtension(selectedobj.file_name);//.xlsx   
        ...
        public async Task<string> GeneratePDFFileAsync(string filename)
        {
            //var filename = selectedobj.file_name;
            var extension = Path.GetExtension(filename);//.xlsx   
            var PdfFileName = filename.Replace(extension, ".pdf");
            var statuscode = "";
            var lstfullname = filename.Split("/");
