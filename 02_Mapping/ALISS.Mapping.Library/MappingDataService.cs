...
namespace ALISS.Mapping.Library
{
    public class MappingDataService : IMappingDataService
    {
        ...
        public List<MappingListsDTO> GetList()
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                ...
                try
                {
                    objList = _db.MappingListDTOs.FromSqlRaw<MappingListsDTO>("sp_GET_TRMappingList").ToList();
                    trans.Commit();
                }
                catch (Exception ex)
                {
        ...
        public MappingDataDTO GetMappingDataActiveWithModel(MappingDataDTO model)
        {
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                	var objDataList = _db.TRMappings.FromSqlRaw<TRMapping>("sp_GET_TRMapping_Approve_Active {0} ,{1} ,{2} ,{3} ,{4}", model.mp_hos_code, model.mp_lab, model.mp_program, model.mp_filetype, model.mp_id).ToList();
                   // var objDataList = new List<TRMapping>();
                   // if (model.mp_program == "MLAB")
                   // {
                   //      objDataList = _db.TRMappings.FromSqlRaw<TRMapping>("sp_GET_TRMapping_Approve_Active {0} ,{1} ,{2} ,{3} ,{4}", model.mp_hos_code, model.mp_lab, model.mp_program, model.mp_filetype, model.mp_id).ToList();
                   // }
                   // else
                   // {
                   //      objDataList = _db.TRMappings.FromSqlRaw<TRMapping>("sp_GET_TRMapping_Approve_Active {0} ,{1} ,{2} ,{3} ", model.mp_hos_code, model.mp_lab, model.mp_program, model.mp_filetype).ToList();
                   // }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        objModel.wdm_localwardname != model.wdm_localwardname)
                        {
                            ...
                            objModel.wdm_hos_code = model.wdm_hos_code;
                            objModel.wdm_updatedate = currentDateTime;
                            chkUpdate = true;
                        }                        
                    }
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    var objReturn1 = _db.TRWardTypeMappings.FirstOrDefault(x => x.wdm_mappingid == model.wdm_mappingid
                                                             ...
                                                             && x.wdm_hos_code == model.wdm_hos_code
        ...
        public TRImportMappingLogDTO SaveTRImportMappingLogData(TRImportMappingLogDTO model)
        {
            log.MethodStart();
        ...
        public List<TempImportMappingLogDTO> SaveTempImportMappingLogData(List<TempImportMappingLogDTO> models)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var result = _db.Database.ExecuteSqlCommand("exec sp_UPDATE_ImportMappingError {0}", "SYSTEM");
