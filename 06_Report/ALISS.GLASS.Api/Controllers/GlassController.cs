...
{  
    ...
    public class GlassController : ControllerBase
    {
        ...
        public IEnumerable<GlassFileListDTO> GetGlassPublicFileListModel([FromBody]GlassSearchDTO searchModel)
        {
        ...
        [HttpPost]
        [Route("api/Glass/GetGlassAreaHealthAnalyzeModel")]
        public IEnumerable<GlassAnalyzeListDTO> GetGlassAreaHealthAnalyzeModel([FromBody]GlassSearchDTO searchModel)
        {
            var objReturn = _service.GetGlassAreaHealthAnalyzeModel(searchModel);

            return objReturn;
        }

        [HttpPost]
        [Route("api/Glass/GetGlassHospitalAnalyzeModel")]
        public IEnumerable<GlassAnalyzeListDTO> GetGlassHospitalAnalyzeModel([FromBody]GlassSearchDTO searchModel)
        {
            var objReturn = _service.GetGlassHospitalAnalyzeModel(searchModel);

            return objReturn;
        }

        [HttpPost]
        [Route("api/Glass/GetGlassProvinceAnalyzeModel")]
        public IEnumerable<GlassAnalyzeListDTO> GetGlassProvinceAnalyzeModel([FromBody]GlassSearchDTO searchModel)
        {
            var objReturn = _service.GetGlassProvinceAnalyzeModel(searchModel);
        ...
        public async Task<IActionResult> TestGenFile()
        {
            ...
            {
                log.MethodStart();
                GlassAnalyzeListDTO model = new GlassAnalyzeListDTO();
                model.gls_year = 2019;
                model.gls_analyze_file_path = "/GLASS/20200929_01_GLASS";
                model.gls_analyze_file_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                model.gls_analyze_file_name = "2019_11_glass.xlsx";
                model.gls_arh_code = "11";
                model.gls_pcr_code = "PCR_202009290015";



                int ParamYear = model.gls_year;
                string ParamAreaHealth = string.Format("Area Health No. {0}", model.gls_arh_code);

                //string sWebRootFolder = @"D:\Project\ALISS_PROJECT\06_Report\ALISS.ANTIBIOGRAM.Api\" + model.analyze_file_path.Remove(0, 1);

                var sFileName = model.gls_analyze_file_name;
                var sFilePdfName = model.gls_analyze_file_name.Replace(".xlsx", "pdf");
                var strReportPath = "";
                ...
                string sWebRootFolder = Path.Combine(strReportPath, model.gls_analyze_file_path.Remove(0, 1));
                ...
                {
                    #region Sheet1 : Data Glass overview
                    ...
                    foreach (var obj in objGlassPathogen)
                    {
                        if (spc_code != obj.spc_code || org_code != obj.org_code)
                        {
                            if (spc_code != "")
                            {
                                ...
                                //// ------------------------------Use XML for add Error Bar -----------------------------------
                                //var wsBars = chartPathogenNS.WorkSheet;
                                //var xdocBars = chartPathogenNS.ChartXml;
                                //var nsmBars = new System.Xml.XmlNamespaceManager(xdocBars.NameTable);

                                //nsmBars.AddNamespace("c", schemaCharts);
                                //nsmBars.AddNamespace("a", schemaDrawingss);

                                //if (nsmBars.HasNamespace("c") == false)
                                //    nsmBars.AddNamespace("c", schemaCharts);

                                //if (nsmBars.HasNamespace("a") == false)
                                //    nsmBars.AddNamespace("a", schemaCharts);

                                //var nodeBars = chartPathogenNS.ChartXml.SelectSingleNode(COLUMN_PATHS, nsmBars);

                                //var errBar = xdocBars.CreateElement("errBars", schemaCharts);
                                //var errBarType = errBar.AppendChild(xdocBars.CreateElement("errBarType", schemaCharts));
                                //var valueattr = errBarType.Attributes.Append(xdocBars.CreateAttribute("val"));
                                //valueattr.Value = "both";
                                //nodeBars.AppendChild(errBar);

                                //var noEndCap = xdocBars.CreateElement("noEndCap", schemaCharts);
                                //valueattr = noEndCap.Attributes.Append(xdocBars.CreateAttribute("val"));
                                //valueattr.Value = "0";
                                //errBar.AppendChild(noEndCap);

                                //var errValType = xdocBars.CreateElement("errValType", schemaCharts);
                                //valueattr = errValType.Attributes.Append(xdocBars.CreateAttribute("val"));
                                //valueattr.Value = "cust";
                                //noEndCap.AppendChild(errValType);
                        ...
                        worksheets.Cells[startRowidx, colPathPSPercentNS].Formula = string.Format("={0}+{1}"
                                                                                                , worksheets.Cells[startRowidx, colPathPSPercentR].Address 
                                                                                                , worksheets.Cells[startRowidx, colPathPSPercentI].Address);
                        //worksheets.Cells[startRowidx, colPathPSPercentR95CI].Value = 0;
                        if (obj.total_drug_test != 0)
                        {

                            worksheets.Cells[startRowidx, colPathPSPercentR95CI].Formula = string.Format("=1.96*(SQRT({0}*{1}/{2}))"
                        ...
                        {
                            worksheets.Cells[startRowidx, colPathPSPercentR95CI].Value = 0;
                            worksheets.Cells[startRowidx, colPathPSPercentNS95CI].Value = 0;
                        }
                    ...
                    const string COLUMN_PATHS = "c:chartSpace/c:chart/c:plotArea/c:barChart/c:ser";
                    string schemaDrawingss = "http://schemas.openxmlformats.org/drawingml/2006/main";
                    string schemaCharts = "http://schemas.openxmlformats.org/drawingml/2006/chart";

                    nsmBars.AddNamespace("c", schemaCharts);
                    nsmBars.AddNamespace("a", schemaDrawingss);
                    nsmBars.AddNamespace("r", "http://schemas.openxmlformats.org/officeDocument/2006/relationships");
                    nsmBars.AddNamespace("c16r2", "http://schemas.microsoft.com/office/drawing/2015/06/chart");
                    ...
                    //var errValType = xdocBars.CreateElement("errValType", schemaCharts);
                    //valueattr = errValType.Attributes.Append(xdocBars.CreateAttribute("val"));
                    //valueattr.Value = "StandardError";
                    //noEndCap.AppendChild(errValType);


                    // ExcelRange hh = worksheets.Cells[(idxRowStart), 8, (idxend), (8)];
                    // //ExcelChartNumericSource nn;
                    // var plus = xdocBars.CreateElement("plus", schemaCharts);
                    // valueattr = plus.Attributes.Append(xdocBars.CreateAttribute("val"));
                    // valueattr.Value = hh.ToString();               
                    ////valueattr = f.AppendChild(xdocBars.CreateElement("f", schemaCharts));                   
                    // //nn.FormatCode = "f";
                    // //nn.ValuesSource = hh;

                    // errValType.AppendChild(plus);
                ...
                var memory = new MemoryStream();
                string contentType = model.gls_analyze_file_type;
        ...
        public async Task<IActionResult> GenerateAnalyzeFile([FromBody]GlassAnalyzeListDTO model)
        {
            try
            {
                log.MethodStart();
                ...
                int ParamYear = model.gls_year;
                string ParamAreaHealth = "";

                if (!string.IsNullOrEmpty(model.gls_hos_code))
                {
                    ParamAreaHealth = string.Format("{0}", model.gls_hos_name);
                }
                else if ((string.IsNullOrEmpty(model.gls_hos_code)) && (!string.IsNullOrEmpty(model.gls_prv_code)))
                {
                    ParamAreaHealth = string.Format("จังหวัด{0}", model.gls_prv_name);
                }               
                else if ((string.IsNullOrEmpty(model.gls_hos_code)) && (string.IsNullOrEmpty(model.gls_prv_code)))
                {
                    ParamAreaHealth = string.Format("Area Health No. {0}", model.gls_arh_name);
                }
                else
                {
