...
namespace ALISS.MasterManagement.Library
{
    public class SpecimenService : ISpecimenService
    {
        ...
        public List<SpecimenDTO> GetList_Mapping_Active_WithModel(SpecimenDTO searchModel)
        {
            log.MethodStart();
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    var objDataList = _db.TCSpecimens.FromSqlRaw<TCSpecimen>("sp_GET_TCSpecimen_Mapping_Active {0}", searchModel.spc_mst_code).ToList();
