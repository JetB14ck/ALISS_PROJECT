                ...
                try
                {
                    ...
                    if (model.mst_status == "N")
                    {
                        ...
                        if (objDataList.Count > 0)
                        {
                            ...
                            var lastCode = lastData.mst_code.Substring(lastData.mst_code.Length -4 , 4);
        ...
        public MasterTemplateDTO SaveCopyData(MasterTemplateDTO model)
        {
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var objCopyDataList = _db.TCMasterTemplates.FromSqlRaw<TCMasterTemplate>("sp_Copy_TCMasterTemplate {0}, {1}, {2}, {3}, {4}", model.mst_code, new_mst_code, model.mst_version, model.mst_date_from, model.mst_createuser).ToList();
